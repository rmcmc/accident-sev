...
labels <- scale_x_discrete(labels=c("Car", 'Bicycle', 'Heavy \n Vehicle', 'Motorcycle',
                             'Other'))
...
labels <- scale_x_discrete(labels=c("Car \n occupant", 'Cyclist', 'Pedestrian', 'Motorcylist',
                             'Other'))
...
```{r, fig.width=8,fig.height=4, fig.cap="Left - Number of severe and slight causalties from accidents by speed limit. Right - Proportion of Severe accidents by speed limit. The risk of severe accident appears to increase with speed until 60mph, after which risk appears to reduce."}
a + b +  plot_layout(guides = "collect", nrow=1) & theme(legend.position = 'top')
```


```{r, fig.width=8,fig.height=4, fig.cap="Cars were involved in the majority of accidents and had the highest number of slight and severe outcomes (Figure 2). However, the proportion of severe outcomes was highest for motorcycle and pedal cycles."}
d + e +  plot_layout(guides = "collect", nrow=1) & theme(legend.position = 'top')
```


```{r, fig.width=8,fig.height=4, fig.cap="Similarly, the number of slight and severe casualties was highest for car occupants (figure 3) but the proportion of severe outcomes was highest for motorcyclists, pedestrians and cyclists. "}
...
table1 <- summary_factorlist(dat, "accident_severity", expl_vars1, 
  ...
  rename(Levels = levels)

caption = "Characteristics of accident related variables by severity, Great Britain, 2020. For dicrete characteristics counts are given with percentage proportions indicated in brackets. For continuous variables (speed and vehicle age) median values are provided, with the Inter Quartile Range (IQR) given in brackets."

knitr::kable(table1, caption = caption)
```
...
| Variable | Reference | Statistical Significant | Effect on Risk | Odds Ratio|
|------------------|------|--------|--------|--------|
|Speed Limit| N/A  | Strong Evidence | Increases | 1.06 |
|Speed Limit Squared| N/A  | Strong Evidence | Increases |  1.00 |
|Road Surface Conditions - Wet | Dry |  No Evidence | No Effect | 0.98 |
|Road Surface Conditions - Other | Dry |  No Evidence | No Effect | 0.86 | 
|Location of Crash - Rural | Urban | Strong Evidence| Increases | 1.43 |
|Trunk Road - Not Trunk | Trunk | Strong Evidence | Increases | 1.31 |
|Time of Day - 10am-4pm | 7am-10am | Some Evidence | Increases | 1.10 |
|Time of Day - 4pm-7pm | 7am-10am | Strong Evidence | Increases | 1.12 |
|Time of Day - 7pm-11pm | 7am-10am | Strong Evidence | Increases | 1.30 |
|Time of Day - 11pm-7am | 7am-10am | Strong Evidence | Increases | 1.82 |
|Age of Oldest vehicle | N/A | Strong Evidence | Increases | 1.01 |
|At least 1 car involved - Yes | No | Strong Evidence | Decreases | 0.73 |
|At least 1 Heavy Vehicle involved - Yes | No | Strong Evidence | Decreases | 0.86 |
|At least 1 Motorcycle involved - Yes | No | No Evidence | No Effect | 0.84 |
|At least 1 Bicycle involved - Yes | No | Weak Evidence | Decreases | 0.47 |
|At least 1 Other vehicle type involved - Yes | No | Strong Evidence | Increases | 1.24 |
|At least 1 Female Driver Involved - Yes | No | Strong Evidence | Decreases | 0.76 |
|At least 1 Under 25 Driver Involved - Yes| No | No Evidence |No Effect| 0.99 |
|Casualty Cyclist - Yes | No | Strong Evidence | Increases | 7.74 |
|Casualty Pedestrian- Yes | No | Strong Evidence | Increases | 6.43 |
|Casualty Car Occupant - Yes | No | Strong Evidence | Increases | 1.63 |
|Casualty Motorcycle - Yes | No | Strong Evidence | Increases | 6.04 |
|Casualty other vehicle type - Yes | No | Strong Evidence | Increases | 2.17 |

Table: Results of fitted logistic regression model.


```{r, fig.cap="Plot showing the change in predicted probability of a severe accident at different speed limits. The plot demonstrates that the chance of a severe accident occurring generally increases for an increase in speed limit, other than at 70mph. The model is able to replicate the decreasing severity risk at 70mph of figure 1"}
...
dat2 |>
  ...
  geom_errorbar(aes(ymin = lci, ymax = uci), width = 0.05, linewidth = 1, colour='dodgerblue3') +
...
To conclude we can see that the speed limit does in fact have a significant effect on accident severity. This is particularly the case in zones where the most vulnerable road users are to be found. Whether or not a driver is under 25 years of age does not have an impact on acident severity. The driver’s gender does not appear to have any impact on accident severity. Road Surface Conditions do have an impact on accident severity. Unsurprisingly, roads are most dangerous between 11pm and 7am, when it is likely to be darkest. Lastly, single-carriageways (60 mph roads) are the most dangerous type of roads in the UK. Therefore, the government could look to reduce the severity of accidents by introducing additional safety measures on single-carriageways, such as speed cameras. 
...
[5] Sam Firke (2023). janitor: Simple Tools for Examining and Cleaning Dirty Data. R package version 2.2.0.
...
[7] Garrett Grolemund, Hadley Wickham (2011). Dates and Times Made Easy with lubridate. Journal of Statistical
Software, 40(3), 1-25. URL https://www.jstatsoft.org/v40/i03/.

[8]Ewen Harrison, Tom Drake and Riinu Ots (2023). finalfit: Quickly Create Elegant Regression Results Tables and
Plots when Modelling. R package version 1.0.6. https://CRAN.R-project.org/package=finalfit
  
[9] Thomas Lin Pedersen (2022). patchwork: The Composer of Plots. R package version 1.1.2.
https://CRAN.R-project.org/package=patchwork

[10] Achim Zeileis, Torsten Hothorn (2002). Diagnostic Checking in Regression Relationships. R News 2(3), 7-10.
URL https://CRAN.R-project.org/doc/Rnews/

\newpage

## Appendix A - Technical Modelling Details 
...
prob_table <- expl_vars2[-c(2,7, 20)] |> 
  set_names() %>% 
  map_dfr(prob_fn, .id = "Variable") |> 
...
captions = "Average predicted probabilities when applying fitted model back to data"

knitr::kable(prob_table, caption=caption) 
```
